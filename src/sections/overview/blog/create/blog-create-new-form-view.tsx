...
export default function BlogCreateNewFormView({ onClose, categories }: IProps) {
  ...
  const NewBlogSchema = Yup.object().shape({
    ...
    category: Yup.string().required('Vui lòng chọn thể loại nội dung'),
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      await axiosInstance.post(`/items/post`, {
        status: 'published',
        title: data.title,
      ...
      mutate();
      reset();
      enqueueSnackbar(`Tạo blog thành công ${data.title} `, {
        variant: 'success',
      });
      preview.onFalse();
